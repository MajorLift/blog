...
The alternative is to treat the model as scaffolding for thinking rather than as an oracle that answers questions. Scaffolding is a [precise term from learning research](https://doi.org/10.1111/j.1469-7610.1976.tb00381.x): temporary structure that lets a learner perform beyond current capability. The scaffold does not do the thinking. It holds the shape of the thinking while the work happens. In practice this means one change that sounds trivial and is not: declaring the cognitive mode before the content, every time.
