...
The redirect target in my case is a commit skill that proposes how to split the changes and runs lint and format before staging. That skill's own final step is a low-level commit command. So the gate cannot ban that command outright, and the first version simply allowed it. This left the whole arrangement trivially bypassable: reproduce the skill's last step by hand and the gate never fires. My agents found this without being asked to. Again, not deviousness. Hand-rolling the final step is the shortest path to the goal, and the gate had accidentally certified it.
